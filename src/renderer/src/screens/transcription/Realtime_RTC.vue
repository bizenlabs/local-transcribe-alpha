...
// const isPartialTranscribing = ref<boolean>(false)

const chunks = ref<Blob[]>([])
const chunksArray: Blob[] = []
// const dataBlob = ref<Blob>([])
...
// async function whisperTranscribeBuffer(file: Blob): Promise<string> {
//   let whisperConfig = {
//     model: 'whisper-1',
//     prompt: 'string',
...
async function onDataAvailable(blob: Blob): Promise<void> {
  return
  // if (isPartialTranscribing.value) {
  //   return
  // }
  // isPartialTranscribing.value = true
  // const buffer = await blob.arrayBuffer()
  // // const mp3chunk = encoder.value?.encodeBuffer(new Int8Array(buffer))
  ...
  // if (mp3chunk) {
  //   const mp3blob: Blob = new Blob([mp3chunk], { type: 'audio/wav' })
  //   chunks.value.push(mp3blob)
  // }

  const bigBlob = new Blob(chunksArray, { type: 'audio/mpeg' })
  console.log('bigBlob', bigBlob.size)
  const file = new File([bigBlob], 'segment.wav', {
    type: 'audio/mpeg'
  })
  const segment = await whisperTranscribe(file)
  console.log('onDataAvailable-done', { segment })
  if (segment) {
    transcript.value = {
      blob,
      text: segment
  ...
  // isPartialTranscribing.value = false
  // let wav = new WaveFile()
  // wav.fromBuffer(await blob.bytes(), true)
  // const mp3blob = new Blob([mp3chunk], { type: 'audio/mpeg' })

  // console.log('onDataAvailable', blob.type)

  // chunks.value.forEach((blob) => blob.arrayBuffer)
  // let superBuffer: ArrayBuffer = new ArrayBuffer(chunks.value.length)
  // superBuffer.
  // const buffer = await blob.arrayBuffer()
  // console.log({ wav: buffer.byteLength })
  // const buffer = await blob.arrayBuffer()
  // console.log({ wav: buffer.byteLength })
  // const file = new File(wav.getSamples(), 'segment.mp3', { type: 'audio/mpeg' })

  // let combinedText = (transcript.value.text += segment)
